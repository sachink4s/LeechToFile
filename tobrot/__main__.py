...
if __name__ == "__main__" :
    ...
    incoming_message_handler = MessageHandler(
        incoming_message_f,
        filters=Filters.command(["leech@ven0m_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    incoming_youtube_dl_handler = MessageHandler(
        ...
        filters=Filters.command(["ytdl@ven0m_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=Filters.command(["upload@@ven0m_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["savethumbnail@ven0m_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command(["clearthumbnail@ven0m_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
